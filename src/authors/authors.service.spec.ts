...
describe('AuthorsService', () => {
  ...
  beforeEach(async () => {
    const modelMock = {
      ...
      find: jest.fn().mockReturnValue({ skip: jest.fn().mockReturnThis(), limit: jest.fn().mockReturnThis(), exec: jest.fn() }),
      countDocuments: jest.fn(),

      findById: jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(mockAuthor) }),
      findByIdAndUpdate: jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(mockAuthor) }),
      findByIdAndDelete: jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(mockAuthor) }),
    };
  ...
  describe('create', () => {
    it('should create an author and update count', async () => {
      ...
      const dto: CreateAuthorDto = { firstName: mockAuthor.firstName, lastName: mockAuthor.lastName };

      model.countDocuments
        .mockReturnValueOnce({ exec: jest.fn().mockResolvedValue(0) })
        .mockReturnValueOnce({ exec: jest.fn().mockResolvedValue(1) });

      const { count: beforeCount } = await service.findAll({ page: 1, limit: 10, search: mockAuthor.firstName });
      ...
      const { count: afterCount } = await service.findAll({ page: 1, limit: 10, search: mockAuthor.firstName });
  ...
  describe('findAll', () => {
    it('should return authors array and count', async () => {
      ...
      model.find.mockReturnValue({ skip: jest.fn().mockReturnThis(), limit: jest.fn().mockReturnThis(), exec: jest.fn().mockResolvedValue(authorsArray) });
      model.countDocuments.mockReturnValue({ exec: jest.fn().mockResolvedValue(authorsArray.length) });

      const { data, count } = await service.findAll({ page: 1, limit: 10, search: undefined });
  ...
  describe('update', () => {
    it('should update and return the author', async () => {
      ...
      expect(model.findByIdAndUpdate).toHaveBeenCalledWith(mockAuthor._id, dto, { new: true });
      expect(result).toEqual(mockAuthor);
    });
  });
